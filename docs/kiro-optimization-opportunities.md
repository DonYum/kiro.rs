...
- Review 后补充的只读核查确认：账号 7/8/9 已因 `MONTHLY_REQUEST_COUNT`
  月度额度耗尽自动禁用，当前生产 `normal_pool` 只剩账号 10 可调度。
- 没有读取或输出生产 `credentials.json` / `config.json` 内容。

## 已定性的生产观察

### 1. 近 2 小时 sticky 日志只落到 `credential_id=10`
...
Review 后，`@m-DEV-2-cc` 通过只读生产日志把该现象定性：账号 7/8/9
均已因 `MONTHLY_REQUEST_COUNT` 月度额度耗尽被自动禁用，当前 `normal_pool`
只有账号 10。此时 round-robin 和 sticky 都落到账号 10 是唯一可用账号下的正确行为，
不是 task #12 回退。

这个案例反而说明 P0 调度可观测性必须补齐：正确答案是"7/8/9 月度额度耗尽，
可调度池只剩 10"，但现在需要人工 SSH 到生产机 grep 日志才能确认。系统应该直接报告
"当时的候选池有哪些、为什么过滤掉哪些账号、最终为什么选 10"。

给 `@yunfeng` 的运营提醒：当前生产实际只有账号 10 在扛全部流量；7/8/9
需要等月度额度重置后自动恢复，或提前补充/刷新可用账号额度。
...
task #12 的根因是靠人工查 `kiro_stats.json` 和日志定位的。当前线上再次看到 2 小时只选
`credential_id=10`。原因已经由 review 只读核查确认：7/8/9 的
`MONTHLY_REQUEST_COUNT` 月度额度耗尽后被自动禁用，`normal_pool` 只剩 10。
这个结论是良性状态，但定位过程仍依赖人工 grep 生产日志，说明系统输出无法直接回答
"为什么只选 10"。
...
- 对"7/8/9 quota exhausted，只有 10 可调度"这类状态，Admin 状态视图能直接显示
  `quota_exceeded` 过滤原因和当前可调度池。
...
本次账号 10 独占已经确认是 7/8/9 月度额度耗尽，不是 Opus 过滤。但同类问题仍然存在：
如果未来账号的订阅信息过期、为空、或被判定不支持 Opus，那么 Opus 流量只走某一个账号也可能是合理结果；
当前系统仍缺少让运维直接确认的输出。
